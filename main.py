...
try:
  ...
  save_plots = ast.literal_eval(results_config["save_plots"])
  show_plots = ast.literal_eval(results_config["show_plots"])
...
if not os.path.exists(path_pred_file):
  error("Predicted file not found!")


create_p_gt_folders(path_gt_file, path_pred_file, dataset_name, classes)
...
results_files_path = "results-" + str(dataset_name)
...
ground_truth_files_list = glob.glob(f'data/ground-truth-{dataset_name}/*.txt')
...
for txt_file in ground_truth_files_list:
  #print(txt_file)f"predicted-{name}"
  file_id = txt_file.split(".txt",1)[0]
  file_id = os.path.basename(os.path.normpath(file_id))
  # check if there is a correspondent predicted objects file
  if not os.path.exists(f"data/predicted-{dataset_name}/" + file_id + ".txt"):
    error_msg = f"Error. File not found: data/predicted-{dataset_name}/" +  file_id + ".txt\n"
  ...
  for line in lines_list:
    try:
      if "difficult" in line:
          ...
          # class_name = classes[int(class_name_id)]
          is_difficult = True
      else:
      ...
      error(error_msg)
    # check if class is in the classes_to_ignore list, if yes skip
...
predicted_files_list = glob.glob(f'data/predicted-{dataset_name}/*.txt')
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    file_id = os.path.basename(os.path.normpath(file_id))
    if class_index == 0:
      if not os.path.exists(f"data/ground-truth-{dataset_name}/" + file_id + ".txt"):
        error_msg = f"Error. File not found: data/ground-truth-{dataset_name}/" +  file_id + ".txt\n"
        error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
        error(error_msg)
